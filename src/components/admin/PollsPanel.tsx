...
const EMPTY_FORM = { title: '', slug: '', prompt: '', description: '', candidateIds: [] as string[] };
...
export default function PollsPanel({ headers, onLogout }: Props) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const res = await fetch(editingId ? `/api/admin/opinion-polls/${editingId}` : '/api/admin/opinion-polls', { method: editingId ? 'PUT' : 'POST', headers, body: JSON.stringify(form) });
  ...
  const edit = (poll: any) => {
    ...
    setForm({ title: poll.title, slug: poll.slug, prompt: poll.prompt, description: poll.description || '', candidateIds: selected });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const action = async (poll: any, actionName: 'publish' | 'close' | 'archive' | 'reset' | 'default') => {
    let note = '';
